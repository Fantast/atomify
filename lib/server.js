...
module.exports = function (args) {
  ...
  if (w == 2) {
    if (!Array.isArray(args.js)) args.js = [args.js];
    if (!Array.isArray(args.css)) args.css = [args.css];

    args.js.forEach(function (js) {
      js.alias = getNormalizedPath(js.alias || js.entry)
    });

    args.css.forEach(function (css) {
      css.alias = getNormalizedPath(css.alias || css.entry)
    });
  } else {
    args.js = args.js[0]
    args.css = args.css[0]
  ...
  http.createServer(function (req, res) {
    ...
    if (w == 2) {
      var served = args.js.some(function (element) {
        if ((element.alias || element.entry) === resource) {
          if (lr) {
            res.setHeader('Content-Type', 'text/javascript')
            res.end(fs.readFileSync(element.output, {encoding: 'utf8'}))
          } else {
            js(element, responder('javascript', res))
          }
          return true;
        }
        return false;
      ...
      served = served || args.css.some(function (element) {
        if ((element.alias || element.entry) === resource) {
          css(element, responder('css', res))
          return true;
        }
        return false;
      });
...
function serveDefaultPage (res, args) {
  ...
  if (args.css.entry) {
    args.css.forEach(function (element) {
      src += '<link rel="stylesheet" href="CSS">'.replace('CSS', element.alias);
    });
  }
  src += '</head><body>'
  args.css.forEach(function (element) {
    src += '<script src="JS"></script>'.replace('JS', element.alias);
  });
  src += '</body></html>'
